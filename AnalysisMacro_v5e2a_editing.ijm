...
var VersionNumber = "v5e2a_editing";//ready-to-test";
...
var parameters_collected_per_channel=8; //this is cyt, nuc, cytbynuc and Norm versions of each. SHould be 8 now?
 ...
 reportstarted, L_CollectDataReportFile, L_ThresholdArray, L_ThresholdCode)
{
	...
	for (i=0; i<L_channels; i++)//these are shorter than the calcium ones if there are many more calcium timepoints
		{
		if(i!=L_CalciumChannel-1) 
			{
			for (j=0;j<compartment.length;j++) 
				{
				if (j==2 && !SomaCompartmentExists[0]) wait(1); //skip the new image generation if there is no soma compartment in channel0 (same for all channels)
				else newImage(L_SignalNamesRGB[i]+ compartment[j]+"Thresholded", "32-bit black", L_numtimepoints[i], numL_cells, 1);
				}
			}//create containers for data L_passing threshold // no data but height cannot be zero - > will crop off at end!
		}
	//***************************************************************************************** 
	...
	{
	 for (j=0; j <compartment.length; j++) // create containers regardless of whether the compartments exist
		{
		if (j==2 && !SomaCompartmentExists[0]) wait(1); //skip the new image generation if there is no soma compartment in channel0 (same for all channels)
		else newImage(L_SignalNamesRGB[L_CalciumChannel-1]+ compartment[j]+ "Thresholded", "32-bit black", L_numtimepoints[L_CalciumChannel-1], numL_cells, 1); 		
		}		
...
//????BUG I have seen NumberOfROIs > CellThresoldStatus.length - when did an extra ROI get added?
				...
				for (CurrentCompartment=0;CurrentCompartment <2; CurrentCompartment++) //NB********************** constrained here to two compartments
					{ //here is the problem the Ca is thresholded, the Ratio is not
					NumeratorData="Ca"+compartment[CurrentCompartment]+ "Thresholded";
					RatioNames[compartment.length*CurrentRatio+CurrentCompartment] ="Ratio"+L_SignalNamesRGB[NumeratorChannel]+"by"+L_SignalNamesRGB[DenominatorChannel]+compartment[CurrentCompartment];
					imageCalculator("Divide create 32-bit", NumeratorData,"AreaNormSignal");
					
					infile=RatioNames[compartment.length*CurrentRatio+CurrentCompartment]+"Thresholded";
					rename(infile); 
					outfile= RatioNames[compartment.length*CurrentRatio+CurrentCompartment]+"ThresholdedCells"; 
					Normoutfile =RatioNames[compartment.length*CurrentRatio+CurrentCompartment]+"NormalisedThresholdedCells"; 
					condensedoutfile=RatioNames[compartment.length*CurrentRatio+CurrentCompartment]+"CondensedNormalisedThresholdedCells";		
...
//BUG HERE - there is no window without extra names, should there be??
				...
				if (isOpen(ArrayName))
					...
					if (L_cellspastthreshold !=0) 
						...
						if (j==0) saveAs("Tiff", outpathkernel +"ShapeFeatures"+"ThresholdedCells"); //double save  0 for backward compatibility 
						close(); //run("Canvas Size...", "width="+numFeatures +" height="+L_cellspastthreshold+" position=Top-Center zero"); //
						selectWindow(ArrayName+  "_NormalisedThresholded"); saveAs("Tiff", outpathkernel +ArrayName+"_NormalisedThresholdedCells"); 
						if (j==0) saveAs("Tiff", outpathkernel +"ShapeFeatures"+"NormalisedThresholdedCells"); //double save  0 for backward compatibility 
						close(); 
						}
					else
						{
...
function saveCellbyTimeFileset(inF, outF, NormoutF, CondensedF, CondenseFac, pathkernel, width, cells, normmethod, basepoints)
{
	...
	if (CondensedF!="")
		...
		run("Select All"); run("Size...", "width="+(width/CondenseFac)+" height="+cells+" average interpolation=Bilinear"); //note Fiji requires bracket around divide, IJ1 did not. Not sure if interpolation is a good idea here
		saveAs("Tiff", pathkernel +CondensedF);  	
		}
	close();
...
for (WellID =0; WellID <lengthOf(L_WellList); WellID++)  // problem here?
		...
		if (!File.exists(filetoopen)) 
			...
			if (NumberOfTimeunits<1)
				{print("WARNING: cannot calculate calcium spikes, fewer timepoints than timepoints to combine - check condense factor parameter");
				close();}
			else 
				{	
...
maxk=maxk+2*compartment.length*L_RatioPairs.length; //2 compartments  with and without normalisation  -> now 3
...
{ 
	...
	maxk=maxk+2*compartments.length*L_RatioPairs.length; //2 compartments  with and without normalisation  -> now 3
...
for (i=0; i<LL_Channels; i++)
	{// populate the 6 signals quantified for each channel- now 8
	...
	ProcessedSignals[4*LL_Channels + i] ="soma";//nucNormalised"; 
	ProcessedSignals[5*LL_Channels + i] ="nucdNormalised"; 
	ProcessedSignals[6*LL_Channels + i] ="bandNormalised"; 
	ProcessedSignals[7*LL_Channels + i] ="somaNormalised"; 
	}
...
ProcessedSignalsPreIndex = newArray(L_maxk);
	...
	if(L_CaChannel!=-1)
		...
		for (i=0; i<L_RatioPairs.length; i++)
			...
			for (j=0;j<compartment.length;j++)		
				{	
				ProcessedSignalsPreIndex[L_maxk-4 + 2*(compartment.length*i+j)]=LL_Channels+compartment.length*i+j; //extend the channel IDs names to include RatioPairs
				ProcessedSignalsPreIndex[L_maxk-4 +1+ 2*(compartment.length*i+j)]=LL_Channels+compartment.length*i+j; //extend the channel IDs names to include RatioPairs
				}
			}
		}
